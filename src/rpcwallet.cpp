...
Value sendmany(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            HelpRequiringPassphrase() + "\n"
                                        ...
                                        "    {\n"
                                        "      \"address\":amount   (numeric) The abp address is the key, the numeric amount in btc is the value\n"
                                        "      ,...\n"
                                        "    }\n"
    ...
    BOOST_FOREACH (const Pair& s, sendTo) {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid ABP address: ") + s.name_);
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    if (!pwalletMain->Unlock(strWalletPass, anonymizeOnly))
        throw JSONRPCError(RPC_WALLET_PASSABPASE_INCORRECT, "Error: The wallet passphrase entered was incorrect.");
...
Value mintzerocoin(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "mintzerocoin <amount>\n"
            "Usage: Enter an amount of Brk to convert to zBrk"
            + HelpRequiringPassphrase());
...
Value spendzerocoin(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 5 || params.size() < 4)
        throw runtime_error(
            ...
            "Overview: Convert zABP (zerocoins) into ABP. \n"
            "amount: amount to spend\n"
            "mintchange: if there is left over ABP (change), the wallet can convert it automatically back to zerocoins [true]\n"
    ...
    if(fHelp || params.empty() || params.size() > 2)
        throw runtime_error(
            "exportzerocoins include_spent ( denomination )\n"
                ...
                "2. \"denomination\"         (integer, optional) Export a specific denomination of zBrk\n"
    ...
    if(fHelp || params.size() == 0)
        throw runtime_error(
            "importzerocoins importdata \n"
                ...
                "\"value\"            (string) the total zBrk value of zerocoin mints that were added\n"
...
Value reconsiderzerocoins(const Array& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw runtime_error(
            "reconsiderzerocoins\n"
                "\nCheck archived zBrk list to see if any mints were added to the blockchain.\n"
