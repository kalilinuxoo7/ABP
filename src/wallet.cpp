...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CZerocoinMint>& vMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to ABP
...
bool CWallet::MintToTxIn(CZerocoinMint zerocoinSelected, int nSecurityLevel, const uint256& hashTxOut, CTxIn& newTxIn, CZerocoinSpendReceipt& receipt)
{
    ...
    receipt.SetStatus("Transaction Mint Started", ZABP_TXMINT_GENERAL);
    ...
    if (!pubCoinSelected.validate()) {
        receipt.SetStatus("the selected mint coin is an invalid coin", ZABP_INVALID_COIN);
        return false;
    }
    ...
    if (!GenerateAccumulatorWitness(pubCoinSelected, accumulator, witness, nSecurityLevel, nMintsAdded, strFailReason)) {
        receipt.SetStatus("Try to spend with a higher security level to include more coins", ZABP_FAILED_ACCUMULATOR_INITIALIZATION);
    ...
    try {
        ...
        if (!spend.Verify(accumulator)) {
            receipt.SetStatus("the new spend coin transaction did not verify", ZABP_INVALID_WITNESS);
            return false;
        }
        ...
        try {
            ...
            receipt.SetStatus("failed to deserialize", ZABP_BAD_SERIALIZATION);
            return false;
        }
        ...
        if (!newSpendChecking.Verify(accumulator)) {
            receipt.SetStatus("the transaction did not verify", ZABP_BAD_SERIALIZATION);
            return false;
        }
        ...
        for (const CBigNum& item : listCoinSpendSerial) {
            if (spend.getCoinSerialNumber() == item) {
                ...
                receipt.SetStatus("the coin spend has been used", ZABP_SPENT_USED_ZABP);
                return false;
            }
        }
        ...
        receipt.SetStatus("CoinSpend: Accumulator witness does not verify", ZABP_INVALID_WITNESS);
        return false;
    }

    receipt.SetStatus("Spend Valid", ZABP_SPEND_OKAY); // Everything okay
...
bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CZerocoinMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CBitcoinAddress* address)
{
    ...
    if (nValue < 1) {
        receipt.SetStatus("Value is below the the smallest available denomination (= 1) of zBrk", nStatus);
        return false;
    }
    ...
    if (vSelectedMints.empty()) {
        ...
        // Select the zBrk mints to use in this spend
    ...
    receipt.SetStatus("Transaction Created", ZABP_SPEND_OKAY); // Everything okay
