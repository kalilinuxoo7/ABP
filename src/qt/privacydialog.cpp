...
void PrivacyDialog::on_pushButtonZBrkControl_clicked()
{
    ZBrkControlDialog* zBrkControl = new ZBrkControlDialog(this);
    zBrkControl->setModel(walletModel);
    zBrkControl->exec();
}

void PrivacyDialog::setZBrkControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBrkSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzABP()
{
    ...
    // use mints from zBrk selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBrkControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBrkControlDialog::GetSelectedMints();
    }
    ...
    ZBrkControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBrk Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Brk, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBrk Mint");
