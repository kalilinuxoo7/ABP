...
std::list<std::string> ZBrkControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBrkControlDialog::listMints;

ZBrkControlDialog::ZBrkControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBrkControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBrkControlDialog::~ZBrkControlDialog()
{
    delete ui;
}

void ZBrkControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBrkControlDialog::updateList()
{
...
void ZBrkControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBrkControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBrk_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBrkControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBrkControlDialog::GetSelectedMints()
{
...
void ZBrkControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
